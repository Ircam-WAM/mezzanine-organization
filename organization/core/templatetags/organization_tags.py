...
from organization.network.models import ActivityStatus, PersonListBlock, Person
...
def get_team_persons(team, status):
    ...
    for status in statuses:
        activities = status.activities.filter(
            teams__in=[team],
            date_to__gte=datetime.date.today()
        )
        for activity in activities:
            if activity.person not in persons:
...
def format_date_fct_of(date_start, date_end):
    ...
    if date_end:
        ...
        if date_start.format(get_format('SHORT_DATE_FORMAT')) ==\
                date_end.format(get_format('SHORT_DATE_FORMAT')):
            date_start = date_start.format(get_format('DATE_EVENT_FORMAT_Y'))
        elif date_start.format(get_format('YEAR_MONTH_FORMAT')) ==\
                date_end.format(get_format('YEAR_MONTH_FORMAT')):
...
def extended_custompage_extra_content(extra_content):
    ...
    if extra_content.choice == ECPDC.LIST_NEWS:
        ...
        context["events"] = events.filter(
            Q(start__gt=datetime.datetime.now()) | Q(end__gt=datetime.datetime.now())
        )
        context["past_events"] = events.filter(
            end__lt=datetime.datetime.now()
        ).order_by("start")
    elif extra_content.choice == ECPDC.LIST_JURY:
        jury = PersonListBlock.objects.filter(title__in=["Jury", "jury"])
        if jury:
            jury_list = Person.objects.filter(
...
def hal_labos_exp(hal_url, hal_researche_structure):
    return hal_url +\
        ...
        hal_researche_structure.replace(' ', '+')
...
def filter_content_model(content_list, model_name):
    ...
    # example call in template : new_content=related_content|filter_content_model:"Article"  # noqa: E501
...
def ytb_iframe(text):
    ...
    #             'allow="accelerometer; autoplay; clipboard-write; encrypted-media; ' +\  # noqa: E501
