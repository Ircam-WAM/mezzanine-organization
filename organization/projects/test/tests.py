...
from organization.projects.models import ProjectTopic, Project, ProjectCall,\
    ProjectDemo, ProjectBlogPage, ProjectResidency, ProjectProgram, ProjectProgramType,\
    ProjectWorkPackage, ProjectPlaylist, LinkType, ProjectLink, ProjectImage,\
    ProjectUserImage, ProjectFile, ProjectBlock, ProjectRelatedTitle,\
    ProjectContact, DynamicContentProject, ProjectPublicData, ProjectPrivateData,\
    ProjectResidencyEvent, ProjectResidencyArticle, ProjectResidencyProducer,\
    ProjectResidencyFile, ProjectResidencyImage, ProjectResidencyUserImage
from organization.magazine.models import Article
from organization.network.models import Team, Organization, Person
...
class URLTests(TestCase):
    ...
    def setUp(self):
        ...
        self.project_ict = Project.objects.create(
            title="ict project",
            topic=self.project_topic,
            validation_status=3
        )
        ...
        self.project = Project.objects.create(
            ...
            user=project_user,
            validation_status=1,
            call=self.project_call
        )
        self.project_demo = ProjectDemo.objects.create(
            title="django",
            project=self.project,
            url="https://wave.ircam.fr/demo/bachotheque/"
        )
        self.project_blog_page = ProjectBlogPage.objects.create(
            project=self.project,
            content='django project blog page'
        )
        self.project_residency = ProjectResidency.objects.create(
            project=self.project,
            validated=True
        )
    ...
    def test_projects_blog_page_url(self):
        response = self.client.get(
            '/projects/blog/' + self.project_blog_page.slug + "/"
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_ict_projects_detail(self):
        response = self.client.get(
            '/ict-projects/' + self.project_ict.slug + '/detail/'
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_ict_projects_public(self):
        response = self.client.get(
            '/calls/' + self.project_call.slug + '/projects/create/public/'
        )
        self.assertEqual(response.status_code, 302)
        self.client.login(username="test", password="test")
        response = self.client.get(
        ...
        self.assertTemplateUsed(
            response,
            "projects/project_ict_create_public_funding.html"
        )
    ...
    def test_ict_edit_public(self):
        ...
        self.assertTemplateUsed(
            response,
            "projects/project_ict_edit_public_funding.html"
        )

    @skip('404 error')
    def test_ict_edit_private(self):
        response = self.client.get(
            '/profile/project/private/' + self.project.slug + '/'
        )
        self.assertEqual(response.status_code, 302)
        self.client.login(username="test", password="test")
        response = self.client.get(
    ...
    def test_project_residency_list(self):
        response = self.client.get(
            '/calls/' + self.project_call.slug + '/residencies/list/'
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_project_residency_detail_view(self):
        response = self.client.get(
            '/calls/' +
            self.project_call.slug +
            '/residencies/' +
            self.project_residency.slug +
            '/detail/'
        )
        self.assertEqual(response.status_code, 200)
...
class ProjectTests(TestCase):
    ...
    def setUp(self):
        ...
        self.project = Project.objects.create(
            ...
            type="internal",
            external_id="A12",
            program=self.project_program,
            program_type=self.project_program_type,
            call=self.project_call,
            lead_team=self.lead_team,
            lead_organization=self.lead_organization,
            website="www.ircam.fr",
            topic=self.project_topic,
            is_archive=False,
            funding="public"
        )
        ...
        self.futur_project = Project.objects.create(
            title='futur project',
            date_from=tomorrow,
            date_to=tomorrow
        )
    ...
    def test_project_retrieval(self):
        ...
        self.assertTrue(
            self.project in Project.objects.filter(title="test project")
        )
        ...
        self.assertTrue(
            self.project in Project.objects.filter(program=self.project_program)
        )
        self.assertTrue(
            self.project in Project.objects
                .filter(program_type=self.project_program_type)
        )
        self.assertTrue(self.project in Project.objects.filter(call=self.project_call))
        self.assertTrue(
            self.project in Project.objects.filter(lead_team=self.lead_team)
        )
        self.assertTrue(
            self.project in Project.objects
                .filter(lead_organization=self.lead_organization)
            )
        self.assertTrue(self.project in Project.objects.filter(website="www.ircam.fr"))
        self.assertTrue(
            self.project in Project.objects.filter(topic=self.project_topic)
        )
    ...
    def test_project_deletion(self):
        ...
        dynamic_content_project = DynamicContentProject.objects.create(
            project=self.project
        )
        ...
        self.assertTrue(
            project_playlist in ProjectPlaylist.objects.filter(project__isnull=True)
        )
        self.assertTrue(
            project_link in ProjectLink.objects.filter(project__isnull=True)
        )
        self.assertTrue(
            project_image in ProjectImage.objects.filter(project__isnull=True)
        )
        self.assertTrue(
            project_user_image in ProjectUserImage.objects.filter(project__isnull=True)
        )
        self.assertTrue(
            project_file in ProjectFile.objects.filter(project__isnull=True)
        )
        self.assertTrue(
            project_block in ProjectBlock.objects.filter(project__isnull=True)
        )
        self.assertTrue(
            project_demo in ProjectDemo.objects.filter(project__isnull=True)
        )
        self.assertTrue(
            project_related_title in ProjectRelatedTitle.objects
            .filter(project__isnull=True)
        )
        self.assertFalse(dynamic_content_project in DynamicContentProject.objects.all())
        self.assertTrue(
            project_blog_page in ProjectBlogPage.objects.filter(project__isnull=True)
        )
        self.assertTrue(
            project_contact in ProjectContact.objects.filter(project__isnull=True)
        )
    ...
    def test_project_fk_deletion(self):
        ...
        self.assertTrue(
            self.project in Project.objects.filter(program_type__isnull=True)
        )
        ...
        self.assertTrue(
            self.project in Project.objects.filter(lead_organization__isnull=True)
        )
...
class ProjectPublicDataTests(TestCase):
    ...
    def setUp(self):
        ...
        self.project = Project.objects.create()
        self.project_public = ProjectPublicData.objects.create(
            project=self.project,
            brief_description="brief_description",
            challenges_description="challenges_description",
            ...
            implementation_start_date=datetime.date.today(),
            implementation_period=datetime.date.today() + datetime.timedelta(days=1),
            implementation_duration=datetime.date.today() + datetime.timedelta(days=1),
            image=img,
    ...
    def test_project_public_data_creation(self):
        ...
        self.assertEqual(
            self.project_public.challenges_description, "challenges_description"
        )
        self.assertEqual(
            self.project_public.technology_description, "technology_description"
        )
        self.assertEqual(
            self.project_public.objectives_description, "objectives_description"
        )
        self.assertEqual(
            self.project_public.resources_description, "resources_description"
        )
    ...
    def test_project_public_data_retrieval(self):
        ...
        self.assertTrue(
            self.project_public in ProjectPublicData.objects
            .filter(project=self.project)
        )
        self.assertTrue(
            self.project_public in ProjectPublicData.objects
            .filter(brief_description="brief_description")
        )
        self.assertTrue(
            self.project_public in ProjectPublicData.objects
            .filter(challenges_description="challenges_description")
        )
        self.assertTrue(
            self.project_public in ProjectPublicData.objects
            .filter(technology_description="technology_description")
        )
        self.assertTrue(
            self.project_public in ProjectPublicData.objects
            .filter(objectives_description="objectives_description")
        )
        self.assertTrue(
            self.project_public in ProjectPublicData.objects
            .filter(resources_description="resources_description")
        )
        self.assertTrue(
            self.project_public in ProjectPublicData.objects
            .filter(image_credits="image_credits")
        )
    ...
    def test_project_public_data_update(self):
        ...
        self.assertEqual(
            1,
            ProjectPublicData.objects
            .filter(brief_description="brief_description").count()
        )
        self.assertEqual(
            0,
            ProjectPublicData.objects.filter(brief_description="brief_des").count()
        )
    ...
    def test_project_public_data_fk_deletion(self):
        self.project.delete()
        self.assertTrue(
            self.project_public in ProjectPublicData.objects.filter(
...
class ProjectPrivateDataTests(TestCase):
    ...
    def setUp(self):
        ...
        self.project = Project.objects.create()
        self.project_private = ProjectPrivateData.objects.create(
            project=self.project,
            description="description",
            funding_programme="funding_programme",
            commitment_letter=letter,
    ...
    def test_project_private_data_retrieval(self):
        self.assertTrue(
            self.project_private in ProjectPrivateData.objects.all()
        )
        self.assertTrue(
            self.project_private in ProjectPrivateData.objects.filter(
                project=self.project
            )
        )
        self.assertTrue(
    ...
    def test_project_private_data_update(self):
        ...
        self.assertEqual(
            1,
            ProjectPrivateData.objects.filter(description="description").count()
        )
        self.assertEqual(
            0,
            ProjectPrivateData.objects.filter(description="des").count()
        )
...
class ProjectResidencyTests(TestCase):
    ...
    def setUp(self):
        ...
        self.project = ProjectResidency.objects.create(
            project=self.project_test,
            artist=self.artist,
            validated=True,
            producer_commitment="producer_commitment"
        )
        ...
        self.residency_event = ProjectResidencyEvent.objects.create(
            residency=self.project,
            event=self.event
        )
        ...
        self.project_residency_article = ProjectResidencyArticle.objects.create(
            residency=self.project,
    ...
    def test_project_residency_retrieval(self):
        ...
        self.assertTrue(
            self.project in ProjectResidency.objects.filter(
                project=self.project_test
            )
        )
        self.assertTrue(
            self.project in ProjectResidency.objects.filter(artist=self.artist)
        )
        self.assertTrue(
            self.project in ProjectResidency.objects.filter(validated=True)
        )
        self.assertTrue(
            self.project in ProjectResidency.objects.filter(
                producer_commitment="producer_commitment"
    ...
    def test_project_residency_update(self):
        ...
        self.assertEqual(
            1,
            ProjectResidency.objects.filter(
                producer_commitment="producer_commitment"
            ).count()
        )
        self.assertEqual(
            0,
            ProjectResidency.objects.filter(producer_commitment="producer_comm").count()
        )
        self.project.save()
        self.assertEqual(
            0,
        ...
        self.assertEqual(
            1,
            ProjectResidency.objects.filter(
                producer_commitment="producer_comm"
            ).count()
        )
    ...
    def test_project_residency_fk_deletion(self):
        ...
        self.assertTrue(
            self.project in ProjectResidency.objects.filter(project__isnull=True)
        )
        self.artist.delete()
        self.assertTrue(
            self.project in ProjectResidency.objects.filter(artist__isnull=True)
        )

    def test_project_residency_deletion(self):
        project_residency_producer = ProjectResidencyProducer.objects.create(
            residency=self.project
        )
        project_residency_file = ProjectResidencyFile.objects.create(
            residency=self.project
        )
        project_residency_image = ProjectResidencyImage.objects.create(
            residency=self.project
        )
        project_residency_user_image = ProjectResidencyUserImage.objects.create(
            residency=self.project
        )
        self.project.delete()
        self.assertTrue(
            project_residency_producer in ProjectResidencyProducer.objects.filter(
                residency__isnull=True
            )
        )
        self.assertTrue(
            project_residency_file in ProjectResidencyFile.objects.filter(
                residency__isnull=True
            )
        )
        self.assertTrue(
            project_residency_image in ProjectResidencyImage.objects.filter(
                residency__isnull=True
            )
        )
        self.assertTrue(
            project_residency_user_image in ProjectResidencyUserImage.objects.filter(
                residency__isnull=True
            )
        )
        self.assertTrue(
            self.project_residency_article in ProjectResidencyArticle.objects.filter(
                residency__isnull=True
            )
        )
        self.assertTrue(
            self.residency_event in ProjectResidencyEvent.objects.filter(
                residency__isnull=True
            )
        )
