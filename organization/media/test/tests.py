...
from organization.media.models import MediaCategory, Media, Playlist, LiveStreaming,\
    PlaylistMedia, PlaylistRelated, MediaTranscoded, MediaImage
...
class URLTests(TestCase):
    ...
    def setUp(self):
        ...
        self.media = Media.objects.create(
            title="video",
            external_id="A10",
            poster_url="www.ircam.fr",
            category=self.category
        )
        self.playlist = Playlist.objects.create(
            type="audio",
            title="playlist django",
            description="playing django"
        )
    ...
    def test_stream_slug_type_detail_url(self):
        ...
        self.assertTemplateUsed(
            response,
            'media/live_streaming/live_streaming_detail.html'
        )
...
class MediaTests(TestCase):
    ...
    def test_media_deletion(self):
        ...
        self.assertFalse(
            media_transcoded in MediaTranscoded.objects.filter(media__isnull=True)
        )
        self.assertTrue(media_image in MediaImage.objects.filter(media__isnull=True))
        self.assertTrue(
            playlist_media in PlaylistMedia.objects.filter(media__isnull=True)
        )
...
class PlaylistTests(TestCase):
    ...
    def test_playlist_deletion(self):
        ...
        self.assertTrue(
            playlist_media in PlaylistMedia.objects.filter(playlist__isnull=True)
        )
        self.assertTrue(
            playlist_related in PlaylistRelated.objects.filter(playlist__isnull=True)
        )
