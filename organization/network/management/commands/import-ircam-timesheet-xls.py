...
from organization.network.models import PersonActivityTimeSheet, Person
from organization.projects.models import ProjectWorkPackage, Project
...
class IrcamTimeSheet(object):
    ...
    def set_person_activity_timesheet(
        self,
        activity,
        project,
        percentage,
        ...
        on a project
        """
        PersonActivityTimeSheet.objects.get_or_create(
            activity=activity,
            project=project,
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        for sheet in xls.sheets:
            person_register_id = sheet.cell_value(
                xls.register_id_row,
                xls.register_id_col
            )
            ...
            for person in persons:
                ...
                for col_index in range(
                    xls.first_percent_col,
                    xls.first_percent_col + xls.nb_of_month
                ):
                    ...
                    # find the right activities corresponding to
                    ...
                    if not activities:
                        ...
                        for curr_activity in activities:
                            activity_date_range = datetimerange.DateTimeRange(
                                datetime.datetime.combine(
                                    curr_activity.date_from,
                                    datetime.datetime.min.time()
                                ),
                                datetime.datetime.combine(
                                    curr_activity.date_to,
                                    datetime.datetime.min.time()
                                )
                            )
                    ...
                    if not activity:
                        self.logger.info(
                            ...
                            'No activity correponds to period : ' +
                            str(curr_year) +
                            " " +
                            str(month)
                        )
                        ...
                        while sheet.cell_value(
                            project_row_index,
                            xls.first_project_col
                        ) != "Total final":

                            # get percent
                            percent = sheet.cell_value(project_row_index, col_index)\
                                if sheet.cell_value(project_row_index, col_index)\
                                else 0

                            # try to find project
                            project_id_str = sheet.cell_value(
                                project_row_index,
                                xls.first_project_col - 1
                            )
                            ...
                            if end_project_list_row == 0:
                                ...
                                project, is_created = Project.objects.get_or_create(
                                    external_id=str(project_id_str)
                                )
                                if is_created:
                                    project.title = sheet.cell_value(
                                        project_row_index,
                                        xls.first_project_col
                                    )
                                ...
                                if project:
                                    ...
                                    its.set_person_activity_timesheet(
                                        activity,
                                        project,
                                        percent,
                                        month,
                        ...
                        while sheet.cell_value(
                            work_package_row_index,
                            xls.first_project_col
                        ) != "Date entrée":

                            # get project
                            project_external_id = int(
                                sheet.cell_value(
                                    work_package_row_index,
                                    xls.first_project_col - 1
                                )
                            ...
                            project = Project.objects.get(
                                external_id=str(project_external_id)
                            )
                            ...
                            if project:
                                ...
                                if wk_p_str:
                                    ...
                                    for wk_p_num in wk_p_list:
                                        ...
                                        pat = PersonActivityTimeSheet.objects.filter(
                                            activity=activity,
                                            project=project,
                                            month=month,
                                            year=curr_year
                                        ...
                                        # for each PersonActivityTimeSheet link
                                        # work package
                        ...
                        date_accounting_str = sheet.cell_value(
                            dates_row_index,
                            col_index
                        )
                        date_accounting = xlrd.xldate.xldate_as_datetime(
                            date_accounting_str,
                            1
                        ) if date_accounting_str else None
                        date_validation_str = sheet.cell_value(
                            dates_row_index + 1,
                            col_index
                        )
                        date_validation = xlrd.xldate.xldate_as_datetime(
                            date_validation_str,
                            1
                        ) if date_validation_str else None

                        # get all timesheets, function of the activity, month and year
                        pats = PersonActivityTimeSheet.objects.filter(
                            activity=activity,
                            month=month,
                            year=curr_year
                        )
                ...
                self.logger.info(
                    ...
                    '_________________________ Number of record : ' +
                    tr(processing_counter) + ' _________________________'  # noqa: F821
                )
                print(
                    "Person : " + person.title + " | Number of record : " + str(
                        processing_counter
                    )
                )
