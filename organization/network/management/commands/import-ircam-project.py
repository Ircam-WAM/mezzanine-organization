...
from organization.network.models import Organization, Person, Team
from organization.projects.models import Project, ProjectCall
...
class IrcamProjects(object):
    ...
    def set_call_project(self, call):
        if call and self.project.call is None:
            project_call, is_created = ProjectCall.objects.get_or_create(
                name__icontains=call
            )
            if is_created:
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        for row_index in range(
            xls.project_table_1_first_row, xls.project_table_1_last_row
        ):
            ...
            ip.set_date_from(
                xlrd.xldate.xldate_as_datetime(xls.sheet.cell_value(row_index, 4), 1)
            )
            ip.set_date_to(
                xlrd.xldate.xldate_as_datetime(xls.sheet.cell_value(row_index, 5), 1)
            )
        ...
        # Table 2
        for row_index in range(
            xls.project_table_2_first_row, xls.project_table_2_last_row
        ):
