...
class NetworkTests(TC):
    ...
    def setUp(self):
        ...
        self.parent_team = Team.objects.create(name="team1", organization=self.organization, department=self.department)
        self.team = Team.objects.create(name="team2", organization=self.organization, department=self.department, parent=self.parent_team)
    ...
    def test_network_dependencies(self):
        ...
        self.assertEqual(self.person.activities.all()[0].organizations.all()[0].name,"my_organization")
...
class DepartmentTests(TC):
    ...
    def test_department_retrieval(self):
        self.assertTrue(self.department in Department.objects.filter(organization=self.organization))
        self.assertTrue(self.department in Department.objects.filter(name="my department"))
    ...
    def test_department_deletion(self):
        ...
        media_department = MediaDepartment.objects.create(department = self.department, media= media_test)
        self.department.delete()
        self.assertFalse(self.department in Department.objects.all())
        self.assertTrue(department_page in DepartmentPage.objects.filter(department__isnull =True))
        self.assertTrue(media_department in MediaDepartment.objects.filter(department__isnull =True)) 
...
class TeamTests(TC):
    ...
    def test_team_retrieval(self):
        self.assertTrue(self.team in Team.objects.filter(organization=self.organization))
        ...
        self.assertTrue(self.team in Team.objects.filter(hal_researche_structure="hal_researche_structure")) 
...
class OrganizationTests(TC):
    ...
    def test_organization_fk_deletion(self):
        ...
        self.assertTrue(self.organization in Organization.objects.filter(type__isnull=True))
        self.assertTrue(self.organization in Organization.objects.filter(role__isnull=True))       
    ...
    def test_organization_retrieval(self):
        self.assertTrue(self.organization in Organization.objects.filter(type=self.organization_type))
        self.assertTrue(self.organization in Organization.objects.filter(role=self.organization_role))
        self.assertTrue(self.organization in Organization.objects.filter(initials="ORGNZT"))
        self.assertTrue(self.organization in Organization.objects.filter(email="organization@test.fr"))
        self.assertTrue(self.organization in Organization.objects.filter(telephone="0101010101"))
        self.assertTrue(self.organization in Organization.objects.filter(bio="bio"))
        self.assertTrue(self.organization in Organization.objects.filter(is_on_map=True))
        self.assertTrue(self.organization in Organization.objects.filter(is_host=True))
        self.assertTrue(self.organization in Organization.objects.filter(opening_times="daytime"))
        self.assertTrue(self.organization in Organization.objects.filter(subway_access="subway_access"))
        self.assertTrue(self.organization in Organization.objects.filter(validation_status=1))

    def test_organization_update(self):
        self.organization.email="organization@organization.fr"
        self.assertEqual(1,Organization.objects.filter(email="organization@test.fr").count())
        self.assertEqual(0,Organization.objects.filter(email="organization@organization.fr").count())
        self.organization.save()
        self.assertEqual(0,Organization.objects.filter(email="organization@test.fr").count())
        self.assertEqual(1,Organization.objects.filter(email="organization@organization.fr").count())         
    ...
    def test_organization_deletion(self):
        ...
        self.organization_role.delete()
        self.assertTrue(self.organization in Organization.objects.filter(role__isnull = True))        
        organization_linked_block_inline = OrganizationLinkedBlockInline.objects.create(title="organization_linked_block_inline",organization_main=self.organization)
        organization_linked_inline = OrganizationLinkedInline.objects.create(title="organization_linked_inline",organization = self.organization)
        organization_playlist = OrganizationPlaylist.objects.create(organization = self.organization)
        link_type = LinkType.objects.create(name = "test link")
        organization_link = OrganizationLink.objects.create(organization = self.organization,link_type=link_type)
        organization_image = OrganizationImage.objects.create(organization = self.organization)
        organization_block = OrganizationBlock.objects.create(organization = self.organization)
        organization_service = OrganizationService.objects.create(organization = self.organization, image = img)
        organization_event_location = OrganizationEventLocation.objects.create(organization = self.organization)
        organization_contact = OrganizationContact.objects.create(organization = self.organization)
        organization_user_image = OrganizationUserImage.objects.create(organization = self.organization)
        producer_data = ProducerData.objects.create(organization=self.organization,experience_description="experience_description", producer_description="producer_description")
        self.organization.delete()
        self.assertEqual(0,Organization.objects.filter(email="organization@test.fr").count())
        self.assertTrue(organization_linked_block_inline in OrganizationLinkedBlockInline.objects.filter(organization_main__isnull=True))   
        self.assertTrue(organization_playlist in OrganizationPlaylist.objects.filter(organization__isnull=True))   
        self.assertTrue(organization_link in OrganizationLink.objects.filter(organization__isnull=True))   
        self.assertTrue(organization_image in OrganizationImage.objects.filter(organization__isnull=True))   
        self.assertTrue(organization_block in OrganizationBlock.objects.filter(organization__isnull=True))   
        self.assertTrue(organization_linked_inline in OrganizationLinkedInline.objects.filter(organization__isnull=True))   
        self.assertTrue(organization_service in OrganizationService.objects.filter(organization__isnull=True))   
        self.assertTrue(organization_event_location in OrganizationEventLocation.objects.filter(organization__isnull=True))   
        self.assertTrue(organization_contact in OrganizationContact.objects.filter(organization__isnull=True))   
        self.assertTrue(organization_user_image in OrganizationUserImage.objects.filter(organization__isnull=True))   
        self.assertTrue(producer_data in ProducerData.objects.filter(organization__isnull=True))   
...
class PersonTests(TC):
    ...
    def test_person_deletion(self):
        ...
        link_type = LinkType.objects.create(name = "test link")
        person_link = PersonLink.objects.create(person = self.person,link_type=link_type)
        person_list_block_inline = PersonListBlockInline.objects.create(person = self.person)
        self.person.delete()
        self.assertEqual(0,Person.objects.filter(bio="my bio").count())
        self.assertTrue(person_playlist in PersonPlaylist.objects.filter(person__isnull=True))
        ...
        self.assertTrue(person_list_block_inline in PersonListBlockInline.objects.filter(person__isnull =True))
