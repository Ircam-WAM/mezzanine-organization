...
from organization.pages.models import CustomPage, VertigoPage
...
class DynamicContentPageView(Select2QuerySetSequenceView):
    ...
    def get_queryset(self):
        ...
        custompage = CustomPage.objects.all()
        events = Event.objects.all()
        vertigopage = VertigoPage.objects.all()

        if self.q:
            articles = articles.filter(title__icontains=self.q)
            custompage = custompage.filter(title__icontains=self.q)
            vertigopage = vertigopage.filter(title__icontains=self.q)
            events = events.filter(title__icontains=self.q)

        qs = autocomplete.QuerySetSequence(articles, custompage, vertigopage, events)
