...
from organization.agenda.models import Event, EventBlock, EventImage,\
    EventDepartment, EventPerson, LinkType, EventPlaylist, EventLink,\
    EventPeriod, EventTraining, EventRelatedTitle, DynamicContentEvent
...
# Make sure selenium is working : python manage.py test organization.agenda.tests.EventTestsSelenium.test_load_page  # noqa: E501
...
class EventTests(TestCase):
    ...
    def setUp(self):
        ...
        self.event = Event.objects.create(
            title="mon-evenement",
            start=datetime.strptime("2018-01-13", "%Y-%m-%d").date(),
            user=self._user,
    ...
    def test_event_date(self):
        next_event = Event.objects.create(
            title="mon-futur-evenement",
            start=datetime.strptime("2018-01-14", "%Y-%m-%d").date(),
            user=self._user,
            status=CONTENT_STATUS_PUBLISHED
        )
        previous_event = Event.objects.create(
            title="mon-evenement-passe",
            start=datetime.strptime("2018-01-12", "%Y-%m-%d").date(),
            user=self._user,
            status=CONTENT_STATUS_PUBLISHED
        )
        self.assertEqual(
            self.event.get_previous_by_start_date(),
            previous_event
        )
        self.assertEqual(
            self.event.get_next_by_start_date(),
            next_event
        )
    ...
    def test_event_deletion(self):
        ...
        self.assertTrue(
            event_departement in EventDepartment.objects.filter(event__isnull=True)
        )
        ...
        self.assertTrue(
            event_playlist in EventPlaylist.objects.filter(event__isnull=True)
        )
        self.assertTrue(event_period in EventPeriod.objects.filter(event__isnull=True))
        self.assertTrue(
            event_training in EventTraining.objects.filter(event__isnull=True)
        )
        self.assertTrue(
            event_related_title in EventRelatedTitle.objects.filter(event__isnull=True)
        )
...
class EventTestsSelenium(StaticLiveServerTestCase):
    ...
    def test_event_creation(self):
        self.selenium.get(
            '%s%s' % (self.live_server_url, '/admin/mezzanine_agenda/event/')
        )
        self.selenium.driver.find_element_by_link_text('add/').click()
        """
        Filling the needed inputs
        """
        self.selenium.driver.find_element_by_id('id_title_fr')\
            .send_keys('Mon évènement')
        self.selenium.driver.find_element_by_id('id_status_0').click()
        self.selenium.driver.find_element_by_xpath(
            '//*[@id="event_form"]/div/fieldset/div[6]/div/p/a[1]'
        ).click()
        self.selenium.driver.find_element_by_xpath(
            '//*[@id="event_form"]/div/fieldset/div[6]/div/p/a[2]'
        ).click()
        Select(
            self.selenium.driver.find_element_by_id('id_user')
        ).select_by_visible_text('admin')
        ...
        self.selenium.driver.find_element_by_id(
            'id_periods-0-date_from_0'
        ).send_keys('20/04/2021')
        self.selenium.driver.find_element_by_id(
            'id_periods-0-date_from_1'
        ).send_keys('10:07:08')
        self.selenium.driver.find_element_by_id(
            'id_periods-0-date_to_0'
        ).send_keys('21/04/2021')
        self.selenium.driver.find_element_by_id(
            'id_periods-0-date_to_1'
        ).send_keys('10:07:08')
        self.selenium.driver.find_element_by_xpath(
            "//div[@id='blocks-group']/ul[1]/li[1]/a[1]"
        ).click()
        ...
        """)
        self.selenium.driver.find_element_by_id(
            'id_blocks-0-title_fr'
        ).send_keys("Lorem ipsum")
        Select(
            self.selenium.driver.find_element_by_id('id_departments-0-department')
        ).select_by_visible_text('Organisation 1')
        Select(
            self.selenium.driver.find_element_by_id('id_persons-0-person')
        ).select_by_visible_text('Personne 1')
...
class SearchTestsSelenium(StaticLiveServerTestCase):
    ...
    fixtures = [
        '/srv/lib/mezzanine-organization/organization/agenda/fixtures/event.json'
    ]
    ...
    def setUpClass(cls):
        ...
        cls.selenium = WebDriver().Chrome()
        self.selenium.get('%s%s' % (self.live_server_url, '/'))  # noqa: F821
    ...
    def test_search_event(self):
        ...
        try:
            self.selenium.driver.find_element_by_link_text(
                "/agenda/mon-evenement/detail/"
            )
        except NoSuchElementException:
            return False
        return True
